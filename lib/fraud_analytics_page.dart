...
class _FraudAnalyticsPageState extends State<FraudAnalyticsPage> {
  ...
  Widget build(BuildContext context) {
    ...
    // NEW: same isDesktop breakpoint every other page in the portal uses,
    // so the sidebar behaves identically here too.
    ...
    return Scaffold(
      ...
      // NEW: setting `drawer` here is what makes Scaffold automatically
      // draw the hamburger icon in the AppBar above on mobile — no other
      // change to the AppBar itself was needed.
      drawer: isDesktop ? null : Drawer(child: AppSidebar(role: widget.role, currentPage: 'fraud')),
      // NEW: everything from the original `body:` StreamBuilder below is
      // UNCHANGED — it's just now the second child of a Row, with the
      // sidebar as the first child on desktop.
      body: Row(
